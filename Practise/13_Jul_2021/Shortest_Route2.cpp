...
int main() {
	...
	while (t--)
	{
		...
		{
			cin >> b;
			if (b == 1 or a[b] > 0) cout << 0 << " ";
			else
			{
				auto low = lower_bound(all(right), b);
				log(*low);
				auto high = upper_bound(all(left), b);
				log(*high);
				if () cout << min(b - *low, *high - b) << " ";
				else if (*low != 0) cout << b - *low << " ";
				else if (high != left.end()) cout << *high - b << " ";
				else cout << -1 << " ";
